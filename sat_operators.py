...
panels on the cubed sphere. Two implementations:
  ...
  1. build_scalar_sat_fn()     — the original flux-averaging SAT (BROKEN
     on axis-swap edges due to coordinate mismatch)

  2. build_cartesian_sat_fn()  — the new Cartesian-averaged velocity SAT
     that works correctly on ALL 12 edges

The key insight: at axis-swap edges, the normal mass flux from panel A
is u1 = J·v^1 while from panel B it is u2 = J·v^2. These represent
...
Pipeline per edge (pa, ea) <-> (pb, eb):
  ...
  6. Compute consensus mass flux: J · Q^{n,j} · v_j_avg
  7. SAT penalty = -sign · Hv_inv · (flux_own - flux_consensus)

Energy conservation proof:
  Both panels compute flux_consensus from the SAME Cartesian V_avg,
  so sign_a·flux_cons_a + sign_b·flux_cons_b = 0 (same physical flux,
