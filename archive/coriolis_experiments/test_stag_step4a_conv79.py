...
def coriolis_tendency_eq59(v1, v2, f_h, Jh, J1, J2, Pcv, Pvc,
    ...
    The PROVEN energy-neutral symmetrized Coriolis operator.
    """
    # Step 1: u = Jv · v at v-points
    w1 = v1 * J1[None]
    w2 = v2 * J2[None]
    ...
    # Step 3: multiply by Jh^{-1} to get Jh^{-1} P_vh Jv v
    Jh_inv = 1.0 / Jh
    w1_h = w1_h * Jh_inv[None]
    w2_h = w2_h * Jh_inv[None]

    # Term A: V(Jh C w_h)
    JhCw1 = Jh[None] * f_h[None] * w2_h       # +f Jh w2
    JhCw2 = -Jh[None] * f_h[None] * w1_h      # -f Jh w1
    A1, A2 = apply_V(JhCw1, JhCw2, bases_h, project_h)

    # Term B: Jh C V~(w_h)
    Vtw1, Vtw2 = apply_Vtilde(w1_h, w2_h, bases_h, project_h)
    B1 = Jh[None] * f_h[None] * Vtw2           # +f Jh V~w2
    B2 = -Jh[None] * f_h[None] * Vtw1          # -f Jh V~w1

    # Average: (1/2)(A + B)
    c1_h = 0.5 * (A1 + B1)
    c2_h = 0.5 * (A2 + B2)

    # Step 4: interpolate back to v-points
...
def test_energy_neutrality_convergence():
    """
    Test 3c: Coriolis energy neutrality — exact inner product.

    Test Eq. 57 (no V), Eq. 62 (V, cubed-sphere simplification),
    and Eq. 59 (full symmetrized, PROVEN energy-neutral).

    Uses exact inner product v^T (Hv Jv Q) Fv.
    """
    print("\n" + "=" * 65)
    print("TEST 3c: Coriolis energy neutrality — exact inner product")
    print("=" * 65)
    ...
    def compute_ip(v1, v2, dv1, dv2):
        """Exact inner product v^T (Hv Jv Q) Fv and KE."""
        dv1c, dv2c = jax.vmap(
    ...
    all_ok = True
    for label, cor_fn in [
        ("Eq57 (no V)",    lambda v1, v2: coriolis_tendency(
            v1, v2, f_h, Jh, J1, J2, Pcv, Pvc, bases_h, project_h, use_V=False)),
        ("Eq62 (V)",       lambda v1, v2: coriolis_tendency(
            v1, v2, f_h, Jh, J1, J2, Pcv, Pvc, bases_h, project_h, use_V=True)),
        ("Eq59 (symm)",    lambda v1, v2: coriolis_tendency_eq59(
        ...
        for seed in [42, 123, 999, 2025]:
            ...
            dv1_c, dv2_c = cor_fn(v1, v2)
            ip, KE = compute_ip(v1, v2, dv1_c, dv2_c)
            ratio = abs(ip) / KE if KE > 0 else 0.0
            max_ratio = max(max_ratio, ratio)

        status = '✓' if max_ratio < 1e-12 else '✗'
        print(f"  {status} {label:16s}  |v^T HvJvQ Fv|/KE = {max_ratio:.2e}")
        if max_ratio > 1e-12:
            all_ok = False
